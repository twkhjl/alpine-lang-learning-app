...
add column if not exists image_original_filename text not null default '';

alter table public.word_translations
add column if not exists audio_original_filename text not null default '';

create or replace function public.admin_set_word_image(
	p_word_id integer,
	p_image_url text default '',
	p_image_original_filename text default ''
)
...
begin
	update public.words
	set
		image_url = coalesce(btrim(coalesce(p_image_url, '')), ''),
		image_original_filename = coalesce(btrim(coalesce(p_image_original_filename, '')), '')
	where id = p_word_id
...
create or replace function public.admin_set_word_audio(
	...
	p_audio_original_filename text default ''
)
...
begin
	...
	values (
		...
		coalesce(btrim(coalesce(p_audio_original_filename, '')), '')
	)
	on conflict (word_id, language_code) do update
	set
		audio_filename = excluded.audio_filename,
		audio_original_filename = excluded.audio_original_filename
...
begin
	with cleared_words as (
		...
		where btrim(coalesce(image_url, '')) <> ''
			or btrim(coalesce(image_original_filename, '')) <> ''
		returning 1
	)
	select count(*)
	...
	with cleared_translations as (
		...
		where btrim(coalesce(audio_filename, '')) <> ''
			or btrim(coalesce(audio_original_filename, '')) <> ''
		returning 1
	)
	select count(*)
